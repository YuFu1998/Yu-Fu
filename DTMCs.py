...
def statistics(messages, ideaConnection, antiIdeaConnection):
    ...
    agree=0
    disagree=0
    runs=1000 # run the model 100 times and count the number of agree and disagree
...
def output_graph():
    FewideaConnection = [('1', 2)]  # few connection
    FewantiIdeaConnection = [('3', 2)]  # few connection

    AverageideaConnection=[('8', 6)] # average connection
    AverageantiIdeaConnection=[('9', 6)] # average connection

    ManyideaConnection=[('7', 8)] # many connection
    #antiIdeaConnection=[('1', 2), ('3', 2)] # few connection
    messages = []
    ...
    while i <= 40:
        ...
        y1 = get_expected_infection(i, FewideaConnection, FewantiIdeaConnection)
        ExpectedInfection_few.append(y1)

        y2 = get_expected_infection(i, AverageideaConnection, AverageantiIdeaConnection)
        ExpectedInfection_average.append(y2)

        y3 = get_expected_infection(i, ManyideaConnection, FewantiIdeaConnection)
